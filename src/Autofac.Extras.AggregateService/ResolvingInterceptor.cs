                ...
                if (parameters.Any(p => p.ParameterType.IsGenericType))
                {
                    ...
                    methodMap.Add(
                        ...
                        invocation =>
                        {
                            ...
                            var baseFuncType = _systemAssembly.GetType(funcTypeName) ?? throw new NotSupportedException($"Unable to locate function type for dynamic resolution: {funcTypeName}. Ensure your method doesn't have too many parameters to convert to a System.Func delegate.");
